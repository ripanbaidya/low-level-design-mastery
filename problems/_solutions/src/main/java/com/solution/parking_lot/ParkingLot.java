package com.solution.parking_lot;

import com.solution.parking_lot.entities.ParkingSpot;
import com.solution.parking_lot.entities.Ticket;
import com.solution.parking_lot.entities.Vehicle;
import com.solution.parking_lot.strategy.FareCalculator;
...
public class ParkingLot {
    ...
    public Ticket parkVehicle(Vehicle vehicle) {
        ...
        if (spot != null) {
            System.out.println("[ENTRY] " + vehicle.getLicencePlate()
                    + " parked at Spot #" + spot.getSpotNumber()
                    + " [" + spot.getSize() + "]");

            return new Ticket(generateTicketId(), vehicle, spot, LocalDateTime.now());
        }
        System.out.println("[ENTRY] No available spot for " + vehicle.getLicencePlate());
        return null;
    }
    ...
    public BigDecimal unparkVehicle(Ticket ticket) {
        ...
        System.out.printf("[EXIT]  %s | Spot #%d | Duration: %s min | Fare: Rs %.2f%n",
                ticket.getVehicle().getLicencePlate(),
